...
{
  $name=$_GET['searching'];
  $sel=mysqli_query($conn,"select * from user_register where name like '%$name%' ");
}
else
{
...
{
  $name=$_GET['searching'];
  $sql=mysqli_query($conn,"select * from user_register where name like '%$name%' limit $start,$per_page ");
}
else
{
  $sql=mysqli_query($conn,"select * from  user_register limit $start,$per_page");
} 
?>
...
if(isset($_POST['submit']))
{
    $sql=mysqli_query($conn,"select *from user_register");
        ...
        {
            $a1=$check[$i];
            $del=mysqli_query($conn,"delete from user_register where User_Id=$a1"); 
        }
        if ($del) 
        {
        $sql=mysqli_query($conn,"select *from user_register");
        ...
        <div id="content" class="main-content">
            <div class="container">
             ...
             <table class="table table-bordered">
                  <thead>
                     <tr>
                    <th class="text-center"><input type="checkbox" name="check" id="checkAll">&nbsp;<input type="submit" name="submit" class="btn btn-danger" value="All">
                                                    
                    <th class="text-center">First Name</th>
                    <th class="text-center">Last Name</th>
                    <th class="text-center" bgcolor="black">User Name</th>
            ...
            <?php while($row=mysqli_fetch_array($sql)) { ?>
                                            <tbody>
                                                <tr>
                     <td><input type="checkbox" name="allcheck[]" value="<?php echo $row["User_Id"] ?>"> </td>
                                                   
                                                    
                    <td class="text-center"><?php echo $row['fname'];?></td>
                    <td class="text-center"><?php echo $row['lname'];?></td>
                    <td class="text-center" bgcolor="black"><?php echo $row['UserName'];?></td>
                    <td class="text-center"><?php echo $row['Email'];?></td>
                    <td class="text-center"><?php echo $row['Number'];?></td>
                    <!-- <td class="text-center" bgcolor="black"><?php echo $row['Password'];?></td> -->
                    <td class="text-center"><?php echo $row['Gender'];?></td>
                    <td class="text-center">
                    <ul class="table-controls">
                    <li><a href="u_delete.php?id=<?php echo $row['User_Id']; ?>" data-toggle="tooltip" data-placement="top" title="Delete"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></a></li>
                    </ul>
                </td>
        ...
        <tr>
                                        <td colspan="8">
                                           ...
                                           <div class="dt--pagination"><div class="dataTables_paginate paging_simple_numbers" id="zero-config_paginate"><ul class="pagination"><li class="paginate_button page-item previous " id="zero-config_previous">
                                            <?php if($page>1) 
                                            {  ?>  
                                            <a href="user.php?page=<?php echo ($page-1); ?> " aria-controls="zero-config" data-dt-idx="0" tabindex="0" class="page-link">
